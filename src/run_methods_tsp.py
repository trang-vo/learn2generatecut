...
@app.command()
def run_rainbow(data_root: str, nb_cpus: int, model_folder: str, model_name: str = "best_model.pth",
    ...
    result_root = f"../results/{data_folder}/{socket.gethostname()}/{model_folder}_{model_name[:-4]}_{user_callback_type}_{frequent}"
    try:
        if not os.path.isdir(result_root):
            os.makedirs(result_root)
    except:
    ...
    model_path = os.path.join("../logs", model_folder)
    for path in instance_paths:
        log_path = os.path.join(log_folder, "{}.log".format(path.split("/")[-1].split(".")[0]))
