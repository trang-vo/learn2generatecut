...
problem_type="tsp"
cut_type="subtour"
instance_path="../data/evaluate/tsp/tsplib/kroB200.tsp"
model_path="../models/cut_evaluator/subtour_time"
...
python rainbow_solve.py $problem_type $cut_type $instance_path $model_path --model-name=$model_name --user-callback-type=$user_callback_type --frequent=$frequent --terminal-gap=$terminal_gap --result-path=$result_path --display-log --log-path=$log_path --time-limit=$time_limit --no-dfs-search --no-use-cut-detector --cut-detector-path=$cut_detector_path --device=$device
